...
export class WeatherService implements OnInit {
  ...
  private PLACEURL =
    'https://nominatim.openstreetmap.org/search?q=<PLACE>,%20India&format=json';
  ...
  getWeatherData(place: string) {
    this.httpClient
      .get<any[]>(this.PLACEURL.replace('<PLACE>', place))
      .pipe(
        switchMap((locationData) => {
          ...
          if (locationData && locationData.length > 0) {
            ...
            return this.httpClient.get(
              this.WEATHERURL.replace('<LAT>', lat).replace('<LONG>', lon)
            );
          } else {
            return of(null);
          }
      ...
      .subscribe({
        next: (res) => {
          console.log(res);
          if (res) this.data.set(res as WeatherData);
        },
        error: (err) => {
          console.log(err);
        },
